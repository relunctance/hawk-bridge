...
## 🏗️ 三层分离架构总览（2026-04-19 新增）

> **目标：行业顶级记忆系统，彻底治愈 AI Agent 的健忘症**
>
> **三大优先级：①好用（个人） → ②ToB+个人覆盖 → ③100年愿景**
...
**knowledg-hub** 是 hawk-bridge 的外部知识库连接器，通过 Skill 自动采集个人/ToB 企业数据，为 hawk-bridge 提供 RAG 知识底座。
...
autoself（自我进化闭环）已从 7 层扩展为 10 层（8+2），但架构尚未稳定，**暂不纳入 hawk-bridge TODO**。当 autoself 架构稳定后，再考虑与 hawk-bridge 的集成。

autoself 各层与 hawk-bridge 的关系：
- **L0 记忆层** → hawk-bridge + context-hawk（记忆存储）
- **L5 进化层** → soul-force（从执行结果中学习，写回 hawk-bridge）
