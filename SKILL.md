...
### hawk-memory (Go) — HTTP 服务层

OpenClaw 的 Python subprocess 调用和 Hermes 的 handler.py 都通过 HTTP 与 `hawk-memory (Go)` 通信：

```
                    ┌─────────────────────────────────────┐
                    │      hawk-memory (Go) (FastAPI)       │
...
```bash
# 方式1: 直接运行
python3 ~/repos/hawk-memory/server.py

# 方式2: 使用启动脚本
bash ~/repos/hawk-memory/run.sh

# 方式3: 通过 cronjob 常驻后台
nohup python3 ~/repos/hawk-memory/server.py > ~/.hawk/api.log 2>&1 &
```
...
| `HAWK__PYTHON__HTTP_BASE` | `http://127.0.0.1:18368` | hawk-memory Go 地址 |
...
export HAWK__PYTHON__HTTP_BASE=http://127.0.0.1:18368

# 兼容旧方式（deprecated）
export HAWK_PYTHON_HTTP_MODE=true
export HAWK_API_BASE=http://127.0.0.1:18368
```
