      ...
      rotate() {
        ...
        try {
          ...
          if (existsSync(LOG_FILE_BASE)) {
            ...
            fs4.renameSync(LOG_FILE_BASE, rotatedPath);
          }
        } catch {
        }
...
var HybridRetriever = class {
  ...
  async buildNoisePrototypes() {
    if (!hasEmbeddingProvider()) {
      logger.debug("[hawk-bridge] No embedding provider, skipping noise prototypes");
      return;
    }
    const noiseTexts = [
      "\u597D\u7684\uFF0C\u660E\u767D\u4E86",
    ...
    try {
      ...
      logger.warn({ err: e.message }, "[hawk-bridge] Noise prototype embedding failed, noise filter disabled");
    }
  }
  isNoise(embedding) {
  ...
  async search(query, topK, scope, sourceTypes) {
    ...
    if (hasEmbedding) {
      try {
        ...
        logger.warn({ err }, "[hawk-bridge] Vector search failed, falling back to FTS-only");
      }
    }
    logger.info("[hawk-bridge] Running in FTS-only mode (LanceDB native full-text search)");
    try {
      ...
      logger.error({ err }, "[hawk-bridge] FTS search failed");
      return [];
    }
  }
};
...
`);
    ...
    if (m) {
      ...
      try {
        const { writeFileSync: writeFileSync3, mkdirSync: mkdirSync5, existsSync: existsSync6 } = __require("fs");
...
\u63D0\u793A: \u4F7F\u7528 hawk\u786E\u8BA4 N \u5BF9 \u6765\u9A8C\u8BC1\u8BB0\u5FC6\uFF0C\u6216 hawk\u5426\u8BA4 N \u6765\u6807\u8BB0\u4E0D\u53EF\u9760`);
    ...
    if (m) {
      ...
      try {
        const { readFileSync: readFileSync4, existsSync: existsSync6 } = __require("fs");
...
async function healthCheck() {
  ...
  try {
    ...
    const fs4 = __require("fs");
    fs4.accessSync(hawkHome, fs4.constants.W_OK);
    checks.disk = true;
  } catch (e) {
