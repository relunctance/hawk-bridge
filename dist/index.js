...
`);
      ...
      const oldMemory = await db2.getById(targetId);
      if (!oldMemory) {
        event.messages.push(`
${injectEmoji} \u8BB0\u5FC6\u4E0D\u5B58\u5728\u3002
`);
...
async function isDuplicate(text, threshold = DEDUP_SIMILARITY) {
  try {
    ...
    const results = await db2.search(text, 5, 0.85);
...
var captureHandler = async (event) => {
  ...
  try {
    ...
    for (const p of llmItems) {
      if (await isDuplicate(p.text, config.capture.dedupSimilarity)) {
