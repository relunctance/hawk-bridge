...
> 如何用 Go 操作 LanceDB 记忆系统（通过 hawk-memory-api HTTP API）
...
└────────────────────────┬─────────────────────────────────────┘
                         ...
                         ▼
┌──────────────────────────────────────────────────────────────┐
│              hawk-memory-api (FastAPI, port 18360)           │
│                                                              │
│  ┌────────────┐   ┌──────────────┐   ┌──────────────────┐  │
...
// Step 2: 写入 LanceDB（通过 hawk-memory-api 的 memories 端点或直接 REST，见方案 C）
// 目前推荐通过 hawk-memory-api /memories POST 端点写入
// 注意：需要确认 hawk-memory-api 支持直接写入带向量的记忆
```

> ⚠️ **注意**：方案 B 需要 hawk-memory-api 支持直接写入带 embedding 的记忆。
> 当前 `/capture` 端点会自动调用 LLM 提取，不适合高频写入场景。
> 如需方案 B，建议在 hawk-memory-api 中添加 `POST /memories` 直接写入端点。

---

## 方案 C：LanceDB REST Server（极致性能）

绕过整个 hawk-memory-api Python 层，直接用 LanceDB 内置 REST API。
...
### Q: hawk-memory-api 单 worker 并发不够用怎么办？
...
A: hawk-memory-api 支持 `X-Request-ID` 响应头：
