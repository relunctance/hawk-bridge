...
main() {
  ...
  echo ""
  echo "默认配置（开箱即用，无需任何配置）："
  echo "  • Embedding: sentence-transformers (本地 CPU，无 API key)"
  echo "  • LLM 提取: groq (免费 Llama-3，无 API key)"
  echo ""
  echo "可选升级配置："
  echo ""
  echo "  ① Jina 免费 Embedding API"
  echo "     export JINA_API_KEY=你的key"
  echo "     export HTTPS_PROXY=http://你的代理:端口"
  echo ""
  echo "  ② Ollama 本地 GPU Embedding（推荐）"
