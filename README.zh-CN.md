...
hawk-bridge 正在向**永久宪法级记忆**演进 —— 设计目标是保存 50 年甚至更久。

### 核心理念：记忆宪法层级

> 正如国家有：宪法 → 法律 → 行政法规 → 地方规章 → 临时文件
>
> hawk-bridge 记忆有：宪法层 → 生命层 → 周期层 → 事件层 → 工作层
...
1. **宪法层是锚点** — 所有记忆最终成为宪法记忆或逐渐消亡
2. **DARK 文件格式** — 每条记忆 = 一个独立 JSON 文件（永远不依赖数据库格式）
3. **只追加不修改** — 不覆盖，不删除，除非用户明确授权
4. **多副本存储** — GitHub + Gitee + 本地 NAS（无单点故障）
5. **可迁移设计** — 格式可以变更，内容必须存活 100 年

### v2.0+ 规划

- **L0 宪法记忆**：不可变记忆，采用修正案模式更新
- **L1 生命记忆**：人生里程碑追踪，带阶段感知
- **DARK 归档系统**：只追加的 JSON 文件，同步到 Git（GitHub + Gitee 双推）
- **层级晋升引擎**：L3 事件根据访问频率晋升到 L1/L0
- **冷存储管道**：AWS S3 Glacier / 阿里云归档，长期保存
- **格式迁移系统**：每 5 年迁移到新格式，支持完整回滚
